<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateReceptoresNotificacionesTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('receptores_notificaciones', function (Blueprint $table) {
            $table->unsignedBigInteger('notificacion');
            $table->foreign('notificacion')->references('id')->on('notificaciones');
            $table->unsignedBigInteger('receptor');
            $table->foreign('receptor')->references('id')->on('users');
            $table->boolean('leido')->default(false);
            $table->boolean('mostrar')->default(true);
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('receptores_notificaciones');
    }
}
